...
def process_file(file_path, input_json, UUID, user_id, company_code):
    ...
    print(pdf_file,"--------============================================")
    if not input_json["projectType"] == "eg":
        ocr_obj = call_ocr(file_path, ocr_output_base)
        txt_file_path, page_status = ocr_obj.ocr_main()
    else:
        txt_file_path = os.path.join(final_pdf, company_code, input_json["projectName"], os.path.splitext(pdf_file)[0] + '.pdf')
...
def process_request():
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    for component in input_json['components']:
        if component['name'] in ['upload_multiple_files', 'upload_file']:
            is_dir = True
