...
class call_ocr():
                    ...
                    # if i == 'number' and int(j) == 6:
                        ...
                        for child in page:
                            if child.tag == 'image':
                                ...
                                ratio.append(image_area/page_area)
                            if child.tag == 'text':
                                text_count += 1
                        if max((ratio), default=0.0) < 0.125 and text_count >20:
    ...
    def fallback_digital_text(self,output_path, page_no):
        with open(output_path, 'r', encoding='utf-8') as f:
            text_len = len(f.read().split())

            if text_len <=30:
                command = ['pdftotext', '-layout', '-f', str(page_no), '-l', str(page_no), '-nopgbrk', self.inputpath, output_path]
                # Run the command
                subprocess.run(command, check=True)
    ...
    def generate_digital_text(self, page_no):
        ...
        self.fallback_digital_text(output, page_no)
    ...
    def combine_txt_path(self):
        ...
        if os.path.exists(txtfiles_path):
            # for file in sorted(os.listdir(txtfiles_path)):
            for file in  sorted([f for f in os.listdir(txtfiles_path) if not f.endswith("info.txt") and not f.endswith("coordinates.txt") and f.endswith(".txt")],key=lambda x: int(x.rsplit("_page_")[-1].split('.')[0])):
    ...
    def ocr_main(self):
        try:
            ...
            if extension in imgExt:
                return self.process_image(self.inputpath), {"1":"SCAN"}
