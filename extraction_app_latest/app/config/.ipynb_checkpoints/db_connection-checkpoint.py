...
def get_agentic_data(user_id, company_code, agent_name):  
    query = f"SELECT agent_data FROM agent_hub WHERE agent_name = '{agent_name}' AND user_id = '{user_id}' AND company_code = '{company_code}'"
    mydb = connect_db()
    result = execute_query_v2(mydb,query)
    return result

def get_fields_prompt_agentic(config_id, company_code):
    print(config_id,"##$$$------------------@@@@@@@@@@@@")
    query = "SELECT config_prompt from form_fields_master where config_uuid = '" + str(config_id) + "' and company_code = '"+ str(company_code)+"'";
    mydb = connect_db()
    user_prompt = execute_query_v2(mydb,query)
    print(user_prompt,"@@#####===========")
    if user_prompt[0]["config_prompt"]:
        ...
        header = execute_query_v2(mydb,query) 
        ...
        table =execute_query_v2(mydb,query)

        return header, table
    else:
        return [], []

def update_success_processed_status( agent_name, file_name, company_code, user_id):
...
def update_failed_processed_status( agent_name, file_name, company_code, user_id):
...
def update_running_status( agent_name, file_name, company_code, user_id):
