...
def generative_ai_dynamic_prompting(dynamic_data, txt_file_path):
    try:
        ...
        header_prompt = dynamic_data['header']
        ...
        final_input_header = prompting_headers(header_prompt, invoice_text)
        ...
        extracted_header = genai_main(final_input_header)
        ...
        if line_items_prompt:
            ...
            final_input_lineitems = prompting_line_item(line_items_prompt, invoice_text)
            #print("*******************final_input_lineitems****************")
            #print(final_input_lineitems)
            ...
            extracted_lineitems = genai_main(final_input_lineitems)
            ...
            # if clean_extracted_line["line_items"]==[]:
            #     #print("@@@@@@@@@@")
            #     clean_extracted_line["line_items"]=[[]]
            # #print("final clean_extracted_line")
            # #print(clean_extracted_line)
...
def prompting_headers(header_prompt, invoice_text):
    try:
        filename = '/data/digital_workmate_agentic/extraction_app/app/genai/dynamic_prompt_template_headers.txt'
        ...
        return prompt_template_headers
    except Exception as e:
...
def prompting_line_item(lineitem_prompt, invoice_text):
    try:
        filename = '/data/digital_workmate_agentic/extraction_app/app/genai/dynamic_prompt_template_line_items.txt'
        with open(filename, 'r') as file:
        ...
        return prompt_template_lineitem
    except Exception as e:
...
def genai_main(prompt_text):
    payload = {
        "model": "vertex_ai.gemini-1.5-pro",
        "messages": [
            {
                "role": "system",
                "content": "You are an expert in handling all types of invoice data"
            },
            {
                "role": "user",
                "content": prompt_text
